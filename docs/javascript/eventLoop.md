...
**第一波：** 同步代码（主线程）
console.log(1)：直接打印 1。

setTimeout(..., 0)：这是一个宏任务，交给 Web API 计时，完成后放入宏任务队列（标记为 宏1）。

new Promise(...)：构造函数内部的代码是同步执行的。

执行 console.log(4)，打印 4。

执行 resolve()，将该 Promise 状态变为已完成。

.then(...)：由于 Promise 已解决，其回调被放入微任务队列（标记为 微1）。

console.log(7)：直接打印 7。

当前输出： 1, 4, 7 微任务队列： [微1] 宏任务队列： [宏1]

**第二波：** 清空微任务
同步代码执行完毕，事件循环立即检查并清空微任务队列。

执行 微1（即 Promise 的 .then）：

打印 5。

遇到 setTimeout(() => console.log(6), 0)：这是另一个宏任务，放入宏任务队列（标记为 宏2）。

当前输出： 1, 4, 7, 5 微任务队列： 空 宏任务队列： [宏1, 宏2]

**第三波：** 执行宏任务（逐个执行）
宏任务是一个一个执行的，每执行完一个宏任务，都会去检查并清空一次微任务队列。

执行 宏1（第一个 setTimeout）：

打印 2。

遇到 Promise.resolve().then(...)：产生一个新的微任务（标记为 微2）。
...
关键点总结
Promise 构造函数是同步的：所以 4 会在 7 之前打印。

微任务具有高优先级：在第一波同步代码结束时，会先清空所有微任务（打印 5），然后再去碰宏任务（打印 2）。

宏任务间隙检查微任务：在打印 2 之后，由于它产生了一个微任务 3，事件循环会暂停下一个宏任务 6 的执行，先去把 3 打印完。
